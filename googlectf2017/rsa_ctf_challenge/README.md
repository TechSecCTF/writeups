...
If we can craft a padding string such that the resulting PKCS1v1.5 block is a perfect cube, we can simply take the cube root of it and use it as our signature $m$. Then, the validator will compute $m^e \pmod{n} = m^3$, and the result will be our crafted block.
